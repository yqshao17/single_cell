...
from tools import trim, trim_RNA, run_star, sort_sam, make_combined_genome,make_gtf_annotations,generate_STAR_index, rm_dup
import process_rna
import process_atac
...
parser.add_argument('--config', default='config_RNA', help='config file name')
...
if mode == 'all' or mode == 'postproc':
    ...
    run_star(args.genome_dir, args.output_dir, sample, int(args.nthreads), config.PE, config.read_type)
    
    if config.umi_len==0:
        rm_dup(args.output_dir,sample)
        
    print(datetime.datetime.now(), 'Sorting bam...')
    sort_sam(args.output_dir,sample,int(args.nthreads))
    
    if 'RNA' in args.keeptype:
        print(datetime.datetime.now(),'Getting molecular info...')
        process_rna.molecule_info(args.genome_dir, args.output_dir, sample, config, int(args.nthreads))
        print(datetime.datetime.now(),'Getting summary statistics...')
        generate_single_dge_report(args.output_dir,args.genome_dir,'RNA', config.read_type, sample)
        print(datetime.datetime.now(),sample+' is successfully done!')

    if 'ATAC' in args.keeptype:
        ### parsing alignment ###
        print(datetime.datetime.now(),'Getting molecular info...')
        process_atac.molecule_info(args.genome_dir, args.output_dir, sample, config, int(args.nthreads))
        print(datetime.datetime.now(),'Getting summary statistics...')
        ### summary ###
        generate_single_dge_report(args.output_dir,args.genome_dir,'ATAC',config.read_type, sample)
        ### filter bam file by filtered barcode ###
        
        if not os.path.exists(args.output_dir+'/atac_qc'):
            os.makedirs(args.output_dir+'/atac_qc')
        
        #filter_bam(args.output_dir, sample, config, int(args.nthreads)) # very slow!!
