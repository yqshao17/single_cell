...
def barnyard(cell_data,tickstep=10000,s=4,lim=None,ax=None,fig=None):
    ...
    counts1_norm = counts1/sum(counts1)*1000000
    counts2_norm = counts2/sum(counts2)*1000000
    cell_type1 = counts1_norm>(counts2_norm*9)
    cell_type2 = counts2_norm>(counts1_norm*9)
    ...
    if lim==None:
        ...
        lim1 = int(counts1.max()*1.1)
        lim2 = int(counts2.max()*1.1)
    tickstep1=int(float(format(lim1/5, '.1g')))
    tickstep2=int(float(format(lim2/5, '.1g')))
    ax.set_xticks(plt.arange(0,lim1,tickstep1))
    ax.set_yticks(plt.arange(0,lim2,tickstep2))
    ax.set_xticklabels(plt.arange(0,lim1,tickstep2),rotation=90)
    ax.axis([-int(lim1/30.),lim1,-int(lim2/30.),lim2])
    
    #ax.set_xscale('log') # ugly
    #ax.set_yscale('log')
...
def generate_single_dge_report(output_dir,genome_dir, datatype, read_type='R2', sample_name='', read_thresh=None, sublibraries=None):
    ...
    # sparse matrix (n_cell*n_genes), and pd.Series
    ...
    for s in species:
        ...
        species_umi_counts_norm[s] =  species_umi_counts[s]/sum(species_umi_counts[s])*1000000
    ...
    species_umi_counts_norm = pd.DataFrame(species_umi_counts_norm)
    ...
    for s in species:
        ...
        species_assignments.loc[np.where((species_umi_counts_norm[s]/species_umi_counts_norm.sum(1))>0.9)] = s
    ...
    if sublibraries is None:
        ...
        if read_type == 'R1_R2':
            read_type = 'R2'
        fastqfile = output_dir + '/parse_bc/'+sample_name+'_barcode_%s.fq'%read_type
        ...
        with open(output_dir +'/parse_bc/'+ sample_name+'_pipeline_stats.txt') as f: # get fastq_reads
            line=f.readline()
        ...
        stat_dict['Picard Duplication'] = 0.000
        if os.path.exists(output_dir+'/mapping/'+sample_name+'.Aligned.picard.log'):
            picard_df = pd.read_csv(output_dir+'/mapping/'+sample_name+'.Aligned.picard.log', sep='\t',header=0, comment='#', nrows=1)
            picar_dup = picard_df['PERCENT_DUPLICATION'].values[0]
            stat_dict['Picard Duplication'] = picar_dup
    ...
    for s in species:
        ...
        step_depth = int(seq_depth/10)
        gene_counts_subsampled = {0:0}
        umi_counts_subsampled = {0:0}
        subsample_depths = np.array(list(range(0,int(seq_depth),step_depth)) + [seq_depth],dtype=int)
