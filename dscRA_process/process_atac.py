            ...
            if read.is_proper_pair and (not read.is_reverse):
                ...
                if (abs(read.isize) <= max_flen) and (read.mapq >= min_mapq) :
                    ...
                    if next_cell:
                            ...
                            #df.to_csv('test_df.txt',sep='\t',header=True)
                            if not df['umi'][0]: # no umi, no collapse
                                df['umi']=range(df.shape[0])
                                df_collapsed=pd.DataFrame()
                                df_collapsed['gene']=df['gene']
                                df_collapsed['umi']=df['umi']
                                df_collapsed['counts']=1
                            else:
                                df_collapsed = collapse_umis_dataframe(df)
                            #df_collapsed.to_csv('test_df_collapsed.txt',sep='\t',header=True, index=True)
