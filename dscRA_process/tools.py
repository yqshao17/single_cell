...
PICARD_PATH = '/Share/home/zhangqf/usr/picard-tools-2.1.1/picard.jar'
...
def cmd_cutadapt(nthreads,q5,q3,mlen1,mlen2,a1,a2,trimmed_dir,sample,fq1,fq2):
    # get cutadapt cmd with optional parameters
    para = {'-j': str(nthreads), '-q': '%d,%d'%(q5,q3), '-m': '%d:%d'%(mlen1,mlen2), '-a': str(a1), '-A': str(a2)}
    parameter = ''
    for p in para.keys():
        if para[p]:
            parameter+=p+' '+para[p]+' '
    CMD = CUTADAPT_PATH+""" {0} -o {1}/{2}_R1.fq.gz -p {1}/{2}_R2.fq.gz --overlap 7 {3} {4}""".format(parameter, trimmed_dir,sample,fq1,fq2)
    return CMD
...
def trim(output_dir, sample, fq1, fq2,config, nthreads):
    ...
    a2=config.a2
    if a1 or a2:
        CMD = cmd_cutadapt(nthreads,q5,q3,mlen1,mlen2,a1,a2,trimmed_dir,sample,fq1,fq2)
        subprocess.call(CMD+ """ 1>{0}/{1}.log""".format(trimmed_dir,sample), shell=True)
    else:
        subprocess.call("""cp {0} {1}/{2}_R1.fq.gz""".format(fq1, trimmed_dir,sample), shell=True)
        subprocess.call("""cp {0} {1}/{2}_R2.fq.gz""".format(fq2, trimmed_dir,sample), shell=True)
    return

def trim_RNA(output_dir, sample, nthreads, PE):
...
def run_star(genome_dir, output_dir, sample, nthreads, PE=True, read_type='R1_R2'):
    """ Align reads using STAR.
    """
    # check read_type and mapping parameter
    if read_type !='R1_R2' and PE:
        sys.exit("Error: paired-end mapping needs two reads!")
    if read_type =='R1_R2' and (not PE):
        sys.exit("Error: tell me which read for mapping!")
        
    map_dir=output_dir+'/mapping'
    sample_dir=output_dir+'/parse_bc'
    nthreads = int(nthreads)
    if PE==True:
        rc = subprocess.call(STAR_PATH + """ --genomeDir {0}/ --runThreadN {2} --readFilesIn {4}/{3}_barcode_R1.fq {4}/{3}_barcode_R2.fq --outFileNamePrefix {1}/{3}. --outSAMtype BAM SortedByCoordinate """.format(genome_dir, map_dir, nthreads, sample, sample_dir), shell=True)
    else: # read_type == 'R1' or 'R2'
        rc = subprocess.call(STAR_PATH + """ --genomeDir {0}/ --runThreadN {2} --readFilesIn {4}/{3}_barcode_{5}.fq --outFileNamePrefix {1}/{3}. --outSAMtype BAM SortedByCoordinate """.format(genome_dir, map_dir, nthreads, sample, sample_dir, read_type), shell=True)
...
def rm_dup(output_dir, sample):
    """ Remove duplicates with picard if there is no umi.
    """
    # with sorted mapped bam file
    map_dir=output_dir+'/mapping'
    os.system("""mv {0}/{1}.Aligned.sortedByCoord.out.bam {0}/{1}.Aligned.sortedByCoord.out.withdup.bam""".format(map_dir, sample))
    #os.system("""mv {0}/{1}.Aligned.sorted.cor.bam {0}/{1}.Aligned.sorted.cor.withdup.bam""".format(map_dir, sample))
    rc = subprocess.call("java -jar -Xmx4g "+ PICARD_PATH + """ MarkDuplicates INPUT={0}/{1}.Aligned.sortedByCoord.out.withdup.bam OUTPUT={0}/{1}.Aligned.sortedByCoord.out.bam METRICS_FILE={0}/{1}.Aligned.picard.log REMOVE_DUPLICATES=true ASSUME_SORTED=true READ_NAME_REGEX='.+:([0-9]+):([0-9]+):([0-9]+).*' """.format(map_dir, sample), shell=True)
    

def sort_sam(output_dir, sample, nthreads, by='name', intype='bam'):
    """ Sort samfile by header (cell_barcodes+umi or coordinates) 
    """
    map_dir=output_dir+'/mapping'
    nthreads = int(nthreads)
    if by=='name':
        rc = subprocess.call(SAMTOOLS_PATH + """ sort -n -@ {1} -T {0}/{2}.Aligned.sort -o {0}/{2}.Aligned.sorted.bam {0}/{2}.Aligned.sortedByCoord.out.{3}""".format(map_dir, nthreads, sample, intype), shell=True)
    elif by=='coor':
        rc = subprocess.call(SAMTOOLS_PATH + """ sort -@ {1} -T {0}/{2}.Aligned.sort -o {0}/{2}.Aligned.sorted.bam {0}/{2}.Aligned.out.{3}""".format(map_dir, nthreads, sample, intype), shell=True)
