...
makeWindows <- function(genome, blacklist, windowSize = 10e6, slidingSize = 2e6){
    ...
    cs_factor=as.factor(seqnames(chromSizes))
	windowNuc <- lapply(seq_along(windowSplit), function(x){
		message(sprintf("%s of %s", x, length(windowSplit)))
	    chrSeq <- Biostrings::getSeq(genome,chromSizes[which(cs_factor==names(windowSplit)[x])])
	    grx <- windowSplit[[x]]
...
window <- readRSD('result/TM-All/windows.rsd')
frag_bed=import.bed('data/TME-All/SU010_post.fragment.tsv.gz')
frag_bed$RG <- frag_bed$name
#cnaObj <- scCNA(windows, frag_bed, neighbors = 100, LFC = 1.5, FDR = 0.1, force = FALSE, remove = c("chrM","chrX","chrY"))
...
saveRDS(cnaObj, "result/TME-All/SU010_post_cnv.rds")
