...
option_list = list(
    ...
    make_option(c("-t","--core"), type='integer', default=4)
); 
...
read_count <- function(input){
    ...
    for (f in files){
        if (grepl("count",f)){
            ...
            rna_cells = read.table(f, comment.char = "")
...
register(MulticoreParam(opt$core))
...
write.table(deviationScores(dev), paste0(outdir, '/', opt$name, '_dev.txt'), quote=F, sep='\t')

var = computeVariability(dev)
write.table(var, paste0(outdir, '/', opt$name, '_var.txt'), quote=F, sep='\t')
